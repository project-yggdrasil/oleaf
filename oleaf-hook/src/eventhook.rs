...
impl CxxString {
    pub fn new<S: ToString>(data: S) -> Self {
        ...
        if data.len() > 15 {
            Self { size: data.len() as i32, ipl: StdStrImpl { ptr: Box::into_raw(Box::new(data)) as _ } }
        } else {
            let mut buf = [0; 0x10];
            buf.copy_from_slice(data.as_bytes());
            Self { size: data.len() as i32, ipl: StdStrImpl { buf: unsafe { std::mem::transmute(buf) } } }
        }
    }
    
    pub fn c_str<'a>(&self) -> &'a CStr {
        unsafe { // Real gamers hide unsafety
            if self.size < 16 {
...
pub fn send_event_detour(dispatcher: i64, name: i64, unknown: i64) -> i64 {
    unsafe {
        ...
        let handler_addr = std::mem::transmute::<*const (), FnEventHandlerGetter>(EVENT_HANDLER_GETTER_PTR)(dispatcher, *std::mem::transmute::<&CxxString, *const *const ()>(&testname) as i64);
        if handler_addr != 0 {
